---
title: "tidybulk - part of tidyTranscriptomics - Workflow Tryout"
...
se_mini.counts.norm = se_mini.aggr %>% identify_abundant(factor_of_interest = condition) %>% scale_abundance()
```
...
se_mini.counts.norm %>% select(`count`, count_scaled, .abundant, everything())
```
...
se_mini.counts.norm.variable = se_mini.counts.norm %>% keep_variable()
```
</div>
<div class="column-right">
Standard procedure (comparative purpose)
...
se_mini.counts.norm.MDS =
  se_mini.counts.norm %>%
  reduce_dimensions(method="MDS", .dims = 4)
...
se_mini.counts.norm.MDS %>% pivot_sample()  %>% select(contains("Dim"), everything())

se_mini.counts.norm.MDS %>%
	pivot_sample() %>%
  GGally::ggpairs(columns = 4:(4+3), ggplot2::aes(colour=`Cell.type`))
...
se_mini.counts.norm.PCA %>% pivot_sample() %>% select(contains("PC"), everything())
...
se_mini.counts.norm.MDS.rotated =
  se_mini.counts.norm.MDS %>%
	rotate_dimensions(`Dim1`, `Dim2`, rotation_degrees = 45, action="get")
```
</div>
