...
I added several test tidybulk notebooks to the project (from tidybulk paper etc) - they differ in packages called which is detected by renv. It's unlikely all these packages are required for this test notebook (or the latest tidybulk version)
so some "required" packages according to renv can be ignored (pick 1: Snapshot, just using the currently installed packages.)
...
after snapshot choose option 1

(option 2 gives error "Error: package 'nanny' is not available")
renv might detect more requirements than necessary for this notebook
so ignore and redo snapshot - choose 1: Snapshot, just using the currently installed packages.
...
alternative (NOT DONE) load dataset from README (does not work)
...
update: the documentation of adjust_abundance now does have reference to combat..
...
so adjust_abundance does a batch correction on type (single vs paired end reads) in this case
...
this makes sense - perhaps for workflow clarity rename tt_adj (so it does not look like it has ONLY adjusted data)
sorry if this sounds critical - just trying to understand the workflow, and if it could be used as example workflow..

the result message above does clarify how test_differential_abundance is run - which is good
it seems single and paired reads still considered separately..
